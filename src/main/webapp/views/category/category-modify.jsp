...
<body>
    ...
    <div class="board-write-frame">
        <div class="board-area">
        	<div class="board-write-title">게시글 수정</div>
			<form id="my-form" action="updateBoard.me" method="post" enctype="multipart/form-data">
				<input type="hidden" name="boardNo" value="${ boardData.boardNo }">
	            <div class="border-title-area">
	                <div class="board-title">제목</div>
	                <div class="board-title-content" contenteditable placeholder='제목을 입력해 주세요...' name="board_title" id="div-title">${boardData.boardTitle}</div>
	            ...
	            <div class="border-content-area">
	                ...
	                <div>
	                    ...
	                    <div class="border-content-title-frame">
	                        ...
	                        <span class="border-file-name-span">
	                        	<c:choose>
	                        		<c:when test="${not empty boardData.categoryBoardImage}">
	                        			${boardData.categoryBoardImage}
	                        		</c:when>
	                        		<c:otherwise>
	                        			선택된 파일이 없습니다.
	                        		</c:otherwise>
	                ...
	                <div class="content" contenteditable placeholder='내용을 입력해 주세요...' name="board-content" id="div-content">${boardData.boardContent}</div>
    ...
    <jsp:include page="../common/footer.jsp" />
     ...
     <script>     
        ...
        function cancel() {
        	let result = confirm("게시물 수정을 그만하겠습니까?");
        ...
        fileTarget.on('change', function() {
	        ...
	        let boardCategory = document.querySelector("select[name=board_category]").options;
			for (let i=0; i<boardCategory.length; i++) {

			    if (boardCategory[i].value == "${boardData.categoryName}") {
			    	boardCategory[i].selected = true;
			    	selectCategory(i);
			    } 
			}
			
	        let boardGroup = document.querySelector("select[name=board_group]").options;
			for (let i=0; i<boardGroup.length; i++) {
			    if (boardGroup[i].value == "${boardData.smallGroupName}") {
			    	boardGroup[i].selected = true;
			    }
			}
			
			let boardHeadCnt = document.querySelector("select[name=board_max_headcnt]").options;
			for (let i=0; i<boardHeadCnt.length; i++) {
			    if (boardHeadCnt[i].value == "${boardData.maxCount}") {
			    	boardHeadCnt[i].selected = true;
			    }
			}
